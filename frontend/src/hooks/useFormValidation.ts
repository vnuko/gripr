...
import type { RiderInput, TerrainInput, BikeType, RidingStyle, SkillLevel, WeatherCondition } from '../types/rider-input.js';
...
const VALIDATION_RULES = {
  ...
  skillLevel: {
    required: true,
    validValues: ['beginner', 'intermediate', 'advanced', 'expert'] as SkillLevel[],
...
export function useFormValidation(initialValues: Partial<RiderInput & TerrainInput> = {}) {
  const [values, setValues] = useState<Partial<RiderInput & TerrainInput>>(initialValues);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const validateField = useCallback((field: keyof (RiderInput & TerrainInput), value: unknown): string | null => {
  ...
  const validateAll = useCallback((): ValidationResult => {
    ...
    for (const field of Object.keys(VALIDATION_RULES) as (keyof (RiderInput & TerrainInput))[]) {
