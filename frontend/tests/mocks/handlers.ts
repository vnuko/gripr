...
const mockSuccessResponse = {
  baseline: { front: 22, rear: 25, confidence: 47, note: 'Standard weight-based starting point before terrain & setup factors.' },
  terrainAdjusted: { front: 20, rear: 23, confidence: 62, note: 'Adjusted for selected terrain types, weather and riding style.' },
  aiRecommended: {
    front: 21,
    rear: 24,
    confidence: 85,
...
export const handlers = [
  http.post('http://localhost:3000/api/analyze', async ({ request }) => {
...
export const errorHandlers = [
  http.post('http://localhost:3000/api/analyze', () => {
